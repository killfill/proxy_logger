...
httpProxy.on('request', function(data, req, res, info) {
  ...
  logger.debug('-> '+ info.id + ' '+ info.method+ ' '+ info.url+ ' ['+info.request.size+'kb]');
...
httpProxy.on('response', function(data, req, res, info) {
  ...
  logger.debug('   ' + info.id + ' ' + info.statusCode + ' - '+info.statusCodeDesc+' [' +info.response.size+'kb '+info.took+'s]' );
  ...
  //Dont mind about network problems
